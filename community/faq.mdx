...
<Accordion title="Which LLMs are you using?">
  The cloud version.
  For the community version, the choice is up to you! But here are the best options: using OpenAI API (GPT 3.5 Turbo and more recent), or using locally-run open-source models.
  We plan to add support to additional LLM APIs, but for open-source models, here are the ones we recommend: [Starling LM 7B](https://huggingface.co/berkeley-nest/Starling-LM-7B-alpha), [Mistral 7B v0.2 instruct](https://huggingface.co/mistralai/Mistral-7B-Instruct-v0.2).
</Accordion>
